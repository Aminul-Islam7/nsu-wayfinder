					...
					return (
						<Marker key={key} longitude={lng} latitude={lat} anchor="bottom">
							...
							>
								...
								{/* Marker Pin */}
								{showIcon && <div className={`w-6 h-6 rounded-full flex items-center justify-center shadow-md border-2 border-background transition-transform duration-200 group-hover:scale-110 ${isTransit ? 'bg-emerald-500 text-white' : isClassroom ? 'bg-indigo-500 text-white' : 'bg-rose-500 text-white'}`}>{isTransit ? transit_type === 'staircase' ? <StairsIcon className="w-3.5 h-3.5" /> : <ArrowUpDown className="w-3.5 h-3.5" /> : isClassroom ? <GraduationCap className="w-3.5 h-3.5" /> : <MapPin className="w-3.5 h-3.5" />}</div>}
							</div>
						</Marker>
					);
				})}
